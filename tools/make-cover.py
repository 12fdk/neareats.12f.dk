...
Produces an amber→coral gradient card — the app icon's own ramp — with the post
title, a tag chip and the NearEats wordmark. No photo needed, so posts are
self-contained. This is the fallback for when ComfyUI is unavailable; the
weekly job normally generates a photograph instead. build.py expects the
result at images/blog/<slug>.png.
...
W, H = 1200, 630
AMBER = (255, 176, 37)   # icon, pin top
CORAL = (255, 103, 80)   # icon, pin tip
# The bright icon ramp cannot carry white text — white scores 1.8:1 on the amber
# end. Everything on this card is set in the dark ink the buttons use (9.5:1 /
# 6.1:1). See DESIGN.md §2.1.
INK = (42, 21, 0)
WARM_INK = (168, 58, 12)
...
def main():
    ...
    for y in range(H):
        for x in range(W):
            t = (x / W * 0.55) + (y / H * 0.45)
            px[x, y] = lerp(AMBER, CORAL, min(1, t))
    ...
    draw.text((pad + 26, 108), chip, font=f_tag, fill=WARM_INK)
    ...
    draw.text((pad + 76, H - 98), "NearEats", font=bold(44), fill=INK)
